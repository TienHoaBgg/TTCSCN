                    ...
                    <c:forEach var="item" items="${lep}">
                        <div class="col-md-3">
                            <div class="PhongBan-contain__team--img text-center">
                                <c:if test="${quyen != 'Nhân Sự'}" >   <a href="/AdminKMA/detailEmployee?idRFID=${item.idRFID}&idFingerprint=${item.idFingerprint}&idRoom=${r.idRoom}"><img src="./images/${item.image}" alt=""
                                ...
                                <div class="PhongBan-contain__team--name text-center" style="pointer-events: none; 
                                     cursor: default; ">
                                <c:if test="${quyen != 'Nhân Sự' }" >   <a href="/AdminKMA/detailEmployee?idRFID=${item.idRFID}&idFingerprint=${item.idFingerprint}&idRoom=${r.idRoom}">
                                        <h4>${item.fullName}</h4>
                                        <p>${item.positionn}</p>
                                    </a></c:if>
                                <c:if test="${quyen == 'Nhân Sự' }" >    
